...
public class MyTokenArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    private static final Logger getLog = LoggerFactory.getLogger(MyTokenArgumentResolver.class);
    ...
    @Override
    public Object resolveArgument(MethodParameter methodParameter, ModelAndViewContainer modelAndViewContainer, NativeWebRequest nativeWebRequest, WebDataBinderFactory webDataBinderFactory) throws UnsupportedEncodingException {
        log.info("满足条件，进入参数处理方法");

        String accessToken = nativeWebRequest.getHeader("access-token");

//        String decodeToken = null;
//
//
//        decodeToken = URLDecoder.decode(accessToken, "UTF-8");

        //根据请求头解析token字符串 转换成token实体
